...
- **Public URL**: `aviationcortex.com/endorsekit/*` (path-based; this gear's slug is `endorsekit`, abbreviated `edk` in dense contexts). Routed via Cloudflared ingress to a local web process on port `3014`. **Open question**: the specific path segment (`/endorsekit` vs `/edk` vs `/endorsekit`) is pending a portfolio-wide consistency decision raised in currency-hub PR #3 review — may change before launch.
