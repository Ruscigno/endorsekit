...
- **Public URL**: `aviationcortex.com/endorsekit/*` (path-based; this gear's slug is `endorsekit`, abbreviated `edk` in dense contexts per landing-pages brief §4.7.4). Routed via Cloudflared ingress to a local web process on port matching `aviation/docs/implementation_plan.md` §3.1. Path = slug per the path-consistency decision (landing-pages brief §4.3, 2026-06-02). Abbreviation `edk` is for dense contexts (breadcrumb, gear switcher, analytics labels), NOT URLs.
