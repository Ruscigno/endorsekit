...
- **Public URL**: `aviationcortex.com/endorsekit/*` (path-based; this gear's slug is `endorsekit`, abbreviated `edk` in dense contexts per landing-pages brief §4.7.4). Routed via Cloudflared ingress to a local web process on port matching `aviation-cortex/docs/implementation_plan.md` §3.1. Path = slug per the path-consistency decision (landing-pages brief §4.3, 2026-06-02). Abbreviation `edk` is for dense contexts (breadcrumb, gear switcher, analytics labels), NOT URLs.
...
- **[SUPERSEDED by Aviation Cortex platform integration]** **Single-vendor Supabase: Postgres + Auth + RLS.** The CFI is the only authenticated principal; the student is unauthenticated (signed-token surfaces). Browser uses `@supabase/supabase-js` for both auth and simple CRUD; **RLS is the primary authorization layer** for browser-direct calls. The Go backend connects via `pgxpool`, verifies GoTrue JWTs, and scopes every query by `owner_cfi_id` (research §2.2 / §3).
